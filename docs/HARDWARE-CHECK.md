...
- **The GPU sections read a frequency and a ceiling.** `1900 / 2050 MHz` with
  both numbers plausible means the xe or i915 attribute paths resolved. A
...
- **It enables at all.** The floor moved to GNOME Shell 46 in this version, and
  `MessageTray.Source` takes a `params` object from 46 where 45 took positional
  arguments. An extension that will not enable is the failure mode that change
  could have introduced, and it is loud: the journal says so.
- **The panel renders and is coloured.** Nominal is the panel's own foreground;
  a warning is amber, a confirmed throttle is red. `hide-when-nominal` should
  make the indicator disappear entirely and come back on a warning.
...
- **A screenshot.** `README.md` carries a hand-maintained ASCII mock of the
  ...
  indicator with the popup open, in a non-nominal state — orange or red, so the
