...
- **Nothing under `src/` imports `extension.js` or `prefs.js`.** They are the
  outermost layer, and both hold something an inner one would like — the live
  `Gio.Settings`, the bounds the spin rows use. Reaching for either would invert
  the arrow and put St or Adw in the import graph of a module the Node suite
  loads directly.
...
| `src/domain/temperature.js` | Headroom below a trip point, per channel, each against its own, and the verdict that follows from it: the worse of what the hardware says and what the user asked to be told, worded so it names which of the two spoke. The one thermal judgement that belongs to no particular silicon, so the CPU and AMD GPU rules share it rather than each keeping a copy. |
