...
test('a rule that answers with nothing at all is treated as a broken rule', () => {
    const errors = [];
    // The last two carry a level and would once have passed: the popup draws
    // `summary` and `detail`, so a verdict without them renders
    // "████ CONFIRMED   undefined" — a sentence about the hardware, from a rule
    // that said nothing.
    const answers = [
        undefined, null, 'confirmed', 42, {summary: 'no level'},
        {level: Confidence.CONFIRMED, detail: 'no summary'},
        {level: Confidence.CONFIRMED, summary: '90°C'},
    ];
    ...
    assert.equal(errors.length, answers.length);
...
test('reassessment does not extend the linger', () => {
    const time = clock();
    const monitor = monitorOf(
        [stub('cpu', {verdict: level(Confidence.CONFIRMED)}).component],
        {now: time.now, lingerMs: 1000});

    assert.equal(monitor.poll(THRESHOLDS).lingerUntilMs, 2000);
    ...
    assert.equal(reassessed.level, Confidence.CONFIRMED, 'and the window is still open');
});

// Not extending it and not noticing it has closed are different things, and this
// used to conflate them: the deadline was only ever cleared by a poll, so a
// settings write after the window ran out — a spin button writes one per step —
// answered CONFIRMED out of a window that had closed. A poll at the same instant
// would have said LOW. Two answers about one moment is the failure this whole
// project is built to avoid.
test('reassessment does not report a linger that has run out', () => {
    const time = clock();
    let throttling = true;
    const monitor = monitorOf(
        [stub('cpu', {verdict: () => level(throttling ? Confidence.CONFIRMED : Confidence.LOW)})
            .component],
        {now: time.now, lingerMs: 1000});
    ...
    time.advance(5000);
    const reassessed = monitor.reassess(THRESHOLDS);
    assert.equal(reassessed.lingerUntilMs, null);
    assert.equal(reassessed.level, Confidence.LOW);
    assert.equal(monitor.poll(THRESHOLDS).level, Confidence.LOW,
        'and the poll that follows agrees with it');
});
