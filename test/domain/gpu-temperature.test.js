...
// The CPU rule has said which of the two spoke since the "throttle imminent at a
// number you typed" wording was taken out of it. This rule shared none of that
// code and so said nothing, which left a GPU section turning amber with the
// hardware's own headroom as its only stated reason — a reason that, at 40
// degrees of it, plainly was not the one.
test('a level the hardware did not ask for is attributed to the user', () => {
    const cool = reading([sensor('edge', 90, 130)]);
    assert.match(assessGpuTemperature(cool, null, context).detail,
        /— above your warning threshold\b/);

    const hot = reading([sensor('edge', 96, 130)]);
    assert.match(assessGpuTemperature(hot, null, context).detail,
        /— above your critical threshold\b/);

    // And says nothing when the hardware is the one talking.
    const near = reading([sensor('edge', 95, 100)]);
    assert.doesNotMatch(assessGpuTemperature(near, null, quiet).detail, /threshold/);
});
