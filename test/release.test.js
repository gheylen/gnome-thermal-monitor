...
test('the bug report template asks for the version this ships', () => {
    // The template's placeholder is an example of the answer wanted, so it has
    // to be a version that exists. It is prose about the release living outside
    // the changelog, which is exactly the kind of thing a renumbering forgets —
    // this one said 5 for a while after the manifest said 1.
    const template = read('.github/ISSUE_TEMPLATE/bug_report.yml');
    const placeholder = template.match(/id: extension-version[\s\S]*?placeholder: "e\.g\. (\d+)"/)?.[1];
    assert.equal(Number(placeholder), metadata.version,
        'the extension-version placeholder should name the shipped version');
});
