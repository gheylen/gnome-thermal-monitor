...
// The runtime rule above cannot see this one: a module that imports an entry
// point names no `gi://` itself, so it looks clean while dragging St into the
// graph through the back door. ESLint refuses it in the repository; this is the
// same rule applied to the thing the user actually installs.
test('a module that imports an entry point is reported', async () => {
    const problems = await problemsFor({
        ...SOUND,
        'src/rule.js': "import x from '../prefs.js';\nexport const a = x;\n",
    });
    assert.deepEqual(problems,
        ['src/rule.js imports ../prefs.js, and an entry point is the outermost layer']);
});
