...
export async function checkPackage(root) {
    ...
    for (const [file, specifiers] of importsByFile) {
        ...
        if (!RUNTIME_ALLOWED.has(name)) {
            const runtime = specifiers.filter(isRuntimeSpecifier);
            if (runtime.length > 0)
                problems.push(`${name} imports ${runtime.join(', ')}, which only an adapter may do`);
        }

        // Dependencies point inwards: an entry point imports modules, never the
        // other way round. Checked here as well as by ESLint because the two
        // catch it at different moments — and because an inverted import is
        // invisible to the runtime rule above, which sees only that the module
        // itself named no `gi://`.
        if (ENTRY_POINTS.includes(name)) continue;
        const inverted = specifiers
            .filter(specifier => !isRuntimeSpecifier(specifier))
            .filter(specifier =>
                ENTRY_POINTS.includes(relative(root, resolve(dirname(file), specifier))));
        if (inverted.length > 0)
            problems.push(
                `${name} imports ${inverted.join(', ')}, and an entry point is the outermost layer`);
    }
