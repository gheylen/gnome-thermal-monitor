...
● 92°C          ← orange        — within 10 °C of this CPU's throttle point
...
The zip carries the schema source rather than a compiled one, so unpacking it
into the extensions directory needs the schema compiled once:
...
Installing through extensions.gnome.org or the GNOME Extensions application does
this for you.
...
| GPU shows no data | Check `lspci -k` — is `xe` or `i915` listed as the driver? |
