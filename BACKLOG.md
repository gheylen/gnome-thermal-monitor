...
Every entry under *Deferred work* now needs the same thing: hardware, or a
running GNOME session, or a person who wants to translate it. None of them is
blocked on somebody finding the time to write code.
[`docs/HARDWARE-CHECK.md`](docs/HARDWARE-CHECK.md) is where the first two kinds
get done, and it names the three that are cheapest to close during a pass that
is happening anyway.
...
(orange or red), saved as `assets/screenshot.png` — under about 300 KB, so the
