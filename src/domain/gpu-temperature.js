...
export function assessGpuTemperature(reading, _previous, {thresholds}) {
    ...
    // The preference is judged against this GPU's own tightest channel rather
    // than against the panel number, because unlike the CPU there is no panel
    // number for it. "Tell me at 88°C" is then a statement about a temperature
    // this section is already showing.
    const {level, clauses} = assessHeadroom(tightest, tightest.tempC, thresholds);
    return {level, summary, detail: [...clauses, clock].filter(Boolean).join(' — ')};
}
