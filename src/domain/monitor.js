...
/**
 * A rule is contractually required to answer with all three fields, and anything
 * else is a broken rule.
 *
 * The whole shape is checked, not just the level, because the two strings are
 * what the popup draws: a verdict carrying only a level renders as
 * "████ CONFIRMED   undefined", which reads as a reading rather than as the
 * failure it is. RULE_FAILED says "No data" instead, which is true.
 */
...
export class Monitor {
    ...
    poll(thresholds) {
        ...
        this.#openLinger(assessed.worst);
    ...
    /**
     * Open the linger window on a confirmed throttle.  Only a poll does this: a
     * reassessment answers the same readings and must not move time along.
     *
     * @param {import('./confidence.js').Level} worst
     */
    #openLinger(worst) {
        if (isThrottling(worst)) this.#lingerUntilMs = this.#now() + this.#lingerMs;
    }

    /**
     * The linger deadline while it is still ahead of the clock, and null from
     * the moment it is not.
     *
     * Closing the window is decided here, where the deadline is read, rather
     * than on the next poll.  A reassessment runs between polls — a spin button
     * writes a setting on every step of it — and one that trusted the stored
     * deadline would answer CONFIRMED out of a window that had already closed,
     * with no poll in between to notice.  The adapter does schedule a poll for
     * the moment the window ends, but that is its own timer keeping the panel
     * fresh; the domain must not need it to be right.
     *
     * @returns {number|null}
     */
    #linger() {
        if (this.#lingerUntilMs !== null && this.#now() >= this.#lingerUntilMs)
            this.#lingerUntilMs = null;
        return this.#lingerUntilMs;
    }
     ...
     */
    #level(worst) {
        return this.#linger() !== null ? Confidence.CONFIRMED : worst;
    }
