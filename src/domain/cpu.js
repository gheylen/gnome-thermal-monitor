...
/**
 * The verdict when no counter moved: how much thermal headroom is left, and
 * what the user asked to be told.
 *
 * These are different questions and the rule keeps them apart. The hardware
 * knows where it throttles — `tempN_crit` is TjMax on coretemp, the HTC trip on
 * k10temp — and headroom against that is a fact. The two settings are a
 * preference: "tell me when it is above 88°C" is a reasonable thing to want and
 * a meaningless thing to call evidence, because 88 °C is 12 degrees of headroom
 * on one laptop and two degrees past the trip point on another. TjMax in the
 * wild runs from 85 °C to 125 °C.
 *
 * So both are evaluated, the worse level wins, and the wording says which one
 * spoke. Before this the rule had only the preference and described it in the
 * language of evidence — "throttle imminent" at a number the user typed.
 *
 * @param {{label: string|null, tempC: number, throttlePointC: number}|null} tightest
 *   The sensor with the least headroom, or null if none can be measured.
 * @param {number|null} tempC  The package temperature: the number on the panel.
 * @param {import('./thresholds.js').Thresholds} thresholds
 * @param {string} summary
 * @returns {import('./monitor.js').Verdict}
 */
function temperatureVerdict(tightest, tempC, thresholds, summary) {
    // The hardware's own statement about the tightest channel: how close it is
    // to the trip point it publishes, and whether it is past the temperature it
    // says it aims to hold. See `src/domain/temperature.js`.
    const hardware = temperatureLevel(tightest);

    // The preference is evaluated on the package temperature, which is the
    // number on the panel. "Tell me at 88°C" is a statement about the figure the
    // user is looking at; applying it to a sensor they cannot see would turn the
    // indicator orange with no visible cause.
    const user = thresholds.isCritical(tempC) ? Confidence.HIGH
        : thresholds.isWarm(tempC) ? Confidence.MEDIUM
            : Confidence.LOW;

    const level = worstOf([hardware, user]);

    // Say the headroom whenever it is known, at every level: "18°C below the
    // throttle point" is the single most useful thing this rule can report, and
    // it is true whether or not anything is wrong.
    const measured = tightest === null ? null : describeHeadroom(tightest);

    // Attribute a level the hardware did not ask for to the person who did.
    const preference = !isWorse(user, hardware) ? null
        : user === Confidence.HIGH ? 'above your critical threshold'
            : 'above your warning threshold';

    const detail = [measured, preference].filter(Boolean).join(' — ');
    return {level, summary, detail: detail || 'Nominal'};
}
...
export function assessCpu(reading, previous, {thresholds}) {
    ...
    return temperatureVerdict(tightest, tempC, thresholds, summary);
}
